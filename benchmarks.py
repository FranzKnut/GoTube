...
class CTRNN_DampedForcedPendulum:
    ...
    def fdyn(self, t=0, x=None):
        ...
        x1p = (
                np.uint64(5419046323626097)
                * (exp(-2 * x3) - 1)
                / (np.uint64(4503599627370496) * (exp(-2 * x3) + 1))
                - x1 / 1000000
        ...
        x3p = (
                np.uint64(7144123746377831)
                * (exp(-2 * x3) - 1)
                / (np.uint64(4503599627370496) * (exp(-2 * x3) + 1))
                - x3 / 1000000
                - np.uint64(5048886837752751)
                * (exp(-2 * x4) - 1)
                / (np.uint64(72057594037927936) * (exp(-2 * x4) + 1))
                + np.uint64(5564385670244745)
                * (exp(-2 * x5) - 1)
                / (np.uint64(4503599627370496) * (exp(-2 * x5) + 1))
        )
        x4p = (
                np.uint64(1348796766312415)
                * (exp(-2 * x4) - 1)
                / (np.uint64(4503599627370496) * (exp(-2 * x4) + 1))
                - np.uint64(3086507593514335)
                * (exp(-2 * x3) - 1)
                / (np.uint64(36028797018963968) * (exp(-2 * x3) + 1))
                - x4 / 1000000
                - np.uint64(2476184452153819)
                * (exp(-2 * x5) - 1)
                / (np.uint64(36028797018963968) * (exp(-2 * x5) + 1))
        )
        x5p = (
                np.uint64(1523758031023695)
                * (exp(-2 * x4) - 1)
                / (np.uint64(18014398509481984) * (exp(-2 * x4) + 1))
                - np.uint64(8060407538855891)
                * (exp(-2 * x3) - 1)
                / (np.uint64(4503599627370496) * (exp(-2 * x3) + 1))
                - x5 / 1000000
                - np.uint64(3139112893264555)
                * (exp(-2 * x5) - 1)
                / (np.uint64(2251799813685248) * (exp(-2 * x5) + 1))
        )
...
class CartPoleMLP:
    ...
    def fdyn(self, t=0, state=None):
        ...
        x = np.tanh(np.dot(state, self.params["w1"]) + self.params["b1"])
        x = np.tanh(np.dot(x, self.params["w2"]) + self.params["b2"])
        action = np.dot(x, self.params["w3"]) + self.params["b3"]
        action = action[0]
        ...
        dxdt = np.stack((x_dot, xacc, theta_dot, thetaacc))
